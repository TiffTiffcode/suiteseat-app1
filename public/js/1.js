...
app.post("/create-business", upload.single("heroImage"), async (req, res) => {
  ...
  if (existing) {
    return res.status(400).json({ message: "This Business name already exists." });
  }
  // Get values from req.body.
  // Note: Your Business model still has a 'values' object.
  // We'll extract properties from req.body and put them into the 'values' object.
  const {
    businessName,
    yourName,
    phoneNumber,
    ...
    businessAddress,
    businessEmail,
    // You might have other fields here based on your form
  } = values;

  // Basic validation: businessName is required for the slug
  if (!businessName) {
  ...
  try {
    // 1. Generate the base slug from the businessName
    ...
    // 2. Check for slug uniqueness and append a counter if necessary
    while (await Business.exists({ slug: uniqueSlug })) {
      uniqueSlug = `${baseSlug}-${counter}`;
      counter++;
    }

    // 3. Create the new Business document
    const newBusiness = new Business({
      createdBy: userId,
      values: { // Re-populate the 'values' object as per your schema
        businessName,
        yourName,
        phoneNumber,
        locationName,
      ...
      slug: uniqueSlug, // Assign the generated unique slug
    });
    ...
    // You can add more specific error handling for Mongoose validation errors if needed
...
app.get("/get-client-appointments", async (req, res) => {
  const userId = req.session.userId;
  if (!userId) {
    console.log("❌ Not logged in, returning 401.");
    return res.status(401).json({ message: "Not logged in" });
  }

  try {
    console.log(`🔍 Attempting to find Client for user: ${userId}`);
    const client = await Client.findOne({ createdBy: userId });
console.log("✅ Matched client:", client);
    ...
    const enriched = await Promise.all(
      appointments.map(async (appt) => {
        ...
        try {
          const business = await Record.findById(appt.businessId);
          if (business?.values?.slug) {
            businessSlug = business.values.slug;
          }
        } catch (err) {
    ...
    console.log("✅ Appointments enriched. Sending response.");
    res.json(enriched);

  } catch (err) {
    console.error("❌ Fatal Error loading client appointments:", err);
    res.status(500).json({ message: "Failed to load appointments" });
  }
});
...
app.post("/book-appointment", async (req, res) => {
  ...
  try {
    // ✅ 1. First, try to find existing client tied to session
    let client = await Client.findOne({ createdBy: userId, businessId });

    // ✅ 2. If no client exists, try to fallback to user email
    if (!client) {
      const user = await User.findById(userId);
      if (user?.email) {
        // Check if client already exists under same business + email
        client = await Client.findOne({ businessId, email: user.email });

        // Create if it still doesn't exist
        if (!client) {
          ...
          console.log("♻️ Reused existing client by email:", client._id);
        }
      }
    }

    if (!client) {
      console.log("❌ Still no client profile found or created.");
      return res.status(404).json({ message: "Client not found" });
    }
...
await resend.emails.send({
  ...
  to: client.email,
  subject: `Your Appointment is Confirmed with ${businessName}`,
  html: `
...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const newUser = new User({
      email,
      password, // ✅ Do NOT hash here — your model's pre("save") hook handles it
      firstName,
      lastName,
      phone,
      role
