...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!slug) return showError("No business slug in URL.");
  ...
  try {
    ...
    // Fallback to /public/records
    if (!resp.ok) {
      console.warn("Primary fetch failed", resp.status);
      resp = await fetch(
...
// then proceed to render calendars → categories → services → availability → confirm

// If your data might store a different field name, try them in order:
// ---- pick template from Business and load it ----
const rawTpl =
  v.template ??
  v.templateName ??
  v.bookingTemplate ??
...
function normalizeTemplateKey(x) {
  if (!x) return 'basic-hero';
  const k = String(x).toLowerCase().trim().replace(/\s+/g, '-');
  const alias = { basic: 'basic-hero', default: 'basic-hero', hero: 'basic-hero' };
  return alias[k] || k;
}

const tplKey = normalizeTemplateKey(rawTpl);
console.debug('Using template:', { rawTpl, tplKey });



    // ---- Title (your IDs) ----
    const name =
      v.businessName || v.name || v["Business Name"] || "Business";
    document.title = name;
    const hNew = document.getElementById("bizName");          // <-- your heading
    const hAlt = document.getElementById("business-title");   // alt id if present
    if (hNew) hNew.textContent = name;
    if (hAlt) hAlt.textContent = name;

    // ---- Hero image (your IDs) ----
    const rawHero =
      v.heroImageUrl ?? v.heroImage ?? v["Hero Image"] ?? v.hero_image ?? "";
    const url = toUrl(rawHero);

    const heroNew = document.getElementById("heroImg");        // <-- your hero
    if (heroNew) {
      if (url) {
        heroNew.onload  = () => (heroNew.style.display = "");
        heroNew.onerror = () => (heroNew.style.display = "none");
        heroNew.style.display = "none";
        heroNew.src = url;
      } else {
        heroNew.style.display = "none";
      }
    }
    const heroAlt = document.getElementById("business-hero");  // alt id if present
    if (heroAlt) {
      if (url) {
        heroAlt.onload  = () => (heroAlt.style.display = "");
        heroAlt.onerror = () => (heroAlt.style.display = "none");
        heroAlt.style.display = "none";
        heroAlt.src = url;
      } else {
        heroAlt.style.display = "none";
      }
    ...
    // ---- Calendars load (support either helper) ----
    if (typeof loadCalendarsForBusiness === "function") {
      const calendars = await loadCalendarsForBusiness(biz._id);
      if (typeof renderCalendars === "function") renderCalendars(calendars);
    } else if (typeof loadCalendars === "function") {
      await loadCalendars(biz._id);
    }
    ...
    // also reflect error in the hero title so users see it
...
// ---- CATEGORIES: fetch using many possible field names, with fallbacks ----
...
// ---- SERVICES: fetch using many possible field names, with fallbacks ----
...
async function onSelectCalendar(cal) {
  ...
  STATE.selected.calendarId = cal._id;
  ...
  const cats = await loadCategoriesForCalendar(bizId, cal._id);
...
function renderCategories(categories) {
  ...
  categories.forEach(cat => {
    const name = cat.categoryName || cat.name || cat.Name || "Category";
    const el = document.createElement("button");
    el.className = "card card--select";
    el.style.textAlign = "left";
    el.innerHTML = `<div class="card__title">${escapeHtml(name)}</div>`;
    el.addEventListener("click", () => onSelectCategory(cat._id));
    box.appendChild(el);
  });
}
...
function onSelectTimeslot(timeHHMM) {
  ...
  const match = $$(".timeslot").find((b) => b.textContent === timeHHMM);
