...
async function openAppointmentEditor(apptId) {
  try {
    const res = await fetch(`${API_URL('Appointment')}/${encodeURIComponent(apptId)}?ts=${Date.now()}`, {
    ...
    // show popup and ensure the Business dropdown is populated
    if (typeof openAppointmentPopup === 'function') await openAppointmentPopup();

    // mark edit mode so your submit handler PATCHes instead of POSTing
    ...
    // Wait until businesses are loaded, then set value
    if (apptBizSel) {
      await waitForOptions(apptBizSel, 2, 3000);
      apptBizSel.value = String(businessId || '');
    }

    // Load services/clients for that business, then wait for them to populate
    if (businessId) {
      await Promise.all([
    ...
    // Wait for service list and select the right one
    if (svcSel) {
      await waitForOptions(svcSel, 2, 3000);
      svcSel.value = String(serviceId || '');
      // make sure the selected option carries the calendar id (some save flows rely on it)
    ...
    // Client (loadAppointmentClients may already select it, but set explicitly too)
    if (cliSel) {
